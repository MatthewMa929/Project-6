...
void update()
{
    ...
    if (g_current_scene->m_state.player->collided != nullptr) {
        if (collided->get_entity_type() == ENEMY && (g_current_scene->m_state.player->left_enemy or g_current_scene->m_state.player->right_enemy or g_current_scene->m_state.player->top_enemy) && !g_current_scene->m_state.player->hit)
        {
            g_current_scene->m_state.player->set_position(glm::vec3(5.0f, 0.0f, 0.0f));
            g_current_scene->m_state.player->set_movement(glm::vec3(0.0f));
            g_current_scene->m_state.player->m_speed = 2.5f;
            g_current_scene->m_state.player->set_acceleration(glm::vec3(0.0f, -9.81f, 0.0f));
...
void render()
{
    ...
    if (win and !lose)
    {
        Utility::draw_text(&g_shader_program, g_font_id, "You Win!", 0.4, 0.01f, g_current_scene->m_state.player->get_position());
    }

    if (lose and !win)
    {
        Utility::draw_text(&g_shader_program, g_font_id, "You Lose!", 0.4, 0.01f, g_current_scene->m_state.player->get_position());
    }

    if (g_current_scene == g_Menu && !win) {
        Utility::draw_text(&g_shader_program, g_font_id, "Goku, Press enter to start", 0.4, 0.005f, glm::vec3(1.0f, -3.0f, 0.0f));
    }
